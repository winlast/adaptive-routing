...
def main() -> int:
    ...
    # они уже разошлись со статьями после исправления чисел.
    #
    # PROJECT_STATE в репозиторий не входит и в проверку тоже: он
    # намеренно хранит историю, включая отброшенные величины.
    ...
    # Посадочная страница — самый публичный текст проекта: её видят
    # раньше README. Числа в ней проверяются не по чёрному списку, а
    # поимённо: страница обязана содержать ровно те величины, которые
    # лежат в data/. Иначе однажды она разойдётся с замерами молча.
    raw_page = (BASE / "loadlens/web/index.template.html").read_text(
        encoding="utf-8")
    # Разметку и неразрывные пробелы убираем: иначе проверка ловит не
    # расхождение с данными, а то, каким тегом обёрнуто число.
    page = re.sub(r"<[^>]+>", " ", raw_page).replace("&nbsp;", " ")
    page = re.sub(r"\s+", " ", page.replace("\u00a0", " "))
    cap = load("nginx_capacity.json")
    mod = load("nginx_module.json")["режимы"]
    eco = load("unit_economics.json")["по_порогам"]["100.0"]
    az = load("azure_trace.json")["разброс_p99_к_p50"]
    hand = load("handmade.json")
    for label, text, actual in [
        ("страница: разброс на PostgREST", "72 раза", hand["разница_раз"]),
        ("страница: самый медленный запрос", "123,1 мс",
         max(z["медиана_мс"] for z in hand["замеры"])),
        ("страница: доля функций Azure от 10 раз", "44,6 %",
         az["доля_функций_с_разбросом_от_10_раз_%"]),
        ("страница: доля функций Azure от 100 раз", "11,7 %",
         az["доля_функций_с_разбросом_от_100_раз_%"]),
        ("страница: медиана лёгких, обычная", "103,9 мс",
         mod["обычная (least_conn)"]["лёгкие_медиана_мс"]),
        ("страница: медиана лёгких, наша", "11,3 мс",
         mod["по оценке стоимости"]["лёгкие_медиана_мс"]),
        ("страница: доля медленнее 200 мс, обычная", "36,9 %",
         mod["обычная (least_conn)"]["доля_медленнее_200_мс_%"]),
        ("страница: ёмкость обычной", "37,8",
         cap["режимы"]["обычная (least_conn)"][
             "ёмкость_при_пороге"]["100.0"]["рпс"]),
        ("страница: ёмкость нашей", "59,2",
         cap["режимы"]["по оценке стоимости"][
             "ёмкость_при_пороге"]["100.0"]["рпс"]),
        ("страница: экономия в год", "656 640 ₽",
         eco["разница"]["руб_в_год"]),
        ("страница: экземпляров без нас", "17 серверов вместо 27",
         eco["обычная (least_conn)"]["экземпляров"]),
        ("страница: экземпляров с нами", "17 серверов",
         eco["по оценке стоимости"]["экземпляров"]),
    ]:
        global checked
        checked += 1
        if text not in page:
            problems.append(
                f"{label}: на странице нет «{text}», в данных {actual}")
    ...
    public = ["README.md", "README.en.md", "ПРОВЕРЬ_САМ.md", "loadlens/README.md",
              ...
              "thirdparty/AZURE.md", "ЭКОНОМИКА.md",
              # Материалы премии в репозиторий не входят: они внутренние
              # и лежат только у автора. Если папка есть — проверяются
              # заодно, если нет — пропускаются без ошибки.
              "ledentsov/ЗАЯВКА.md", "ledentsov/КОНКУРЕНТЫ.md",
