...
Рассматриваются четыре источника оценки, от самого грубого к точному:
  ...
  * `MeasuredCost` — таблица, снятая профилированием по всей сетке
    значений параметра. В работающей системе она недоступна, так как
    требует останавливающего замера по всем значениям параметра.
    Верхней границей достижимого она при этом НЕ является: измеряемая
    ею блокировка снята при одном сопутствующем запросе, а под полной
    смешанной нагрузкой поведение движка иное. В замерах эта оценка
    действительно не оказалась лучшей, и выдавать её за границу было бы
    неверно.
...
class MeasuredCost(CostEstimator):
    ...
    Показывает, что даёт знание измеренной стоимости каждого запроса
    без всякого моделирования. Границей достижимого не является:
    величина снята при одном сопутствующем запросе и под полной
    нагрузкой описывает движок лишь приближённо.
    """
...
def build_estimator(name: str) -> CostEstimator:
    ...
    в работающей системе недоступна.
    """
    if name == "measured":
