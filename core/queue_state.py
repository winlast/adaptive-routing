...
движке, и оценивает оставшуюся блокировку: для каждого запроса берётся
время, на которое он занимает движок для остальных, и вычитается уже
отработанное время. Сумма даёт ожидаемое время освобождения движка.

Складывается именно блокировка, а не полное время выполнения. Запрос,
полсекунды ожидающий ответа внешнего сервиса, не мешает движку
обслуживать других и веса в очереди почти не имеет; вычисление той же
длительности занимает движок целиком.
...
class QueueTracker:
    ...
    def add(self, worker: str, endpoint: str, param: float | None) -> int:
        ...
        expected = self._estimator.blocking(endpoint, param, worker)
        with self._lock:
            self._counter += 1
            token = self._counter
