"""
Сравнение политик при фиксированном пороге допуска.

Замер вспомогательный. Итоговые выводы работы получены не здесь, а в
`benchmark_budget.py` (кривая компромисса) и `benchmark_equal_share.py`
(сравнение при равной ёмкости): при одном и том же пороге разные оценки
пропускают в асинхронный движок разную долю трафика, поэтому сравниваются
разные рабочие режимы, а не точности оценок. Здесь порог общий, и результат
годится лишь как обзор, а не как основание для выводов.

Модуль остаётся ещё и потому, что в нём живут функции запуска и остановки
шлюза, которыми пользуются остальные замеры.
...
Разброс между прогонами на одной машине велик, поэтому любые числа
отсюда следует читать только вместе с указанным разбросом.
"""
