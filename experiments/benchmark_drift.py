...
Ухудшать при этом надо тот движок, оценка которого используется для
решения. Величина, которую политика берёт из таблицы, — блокировка
**асинхронного** движка: на сколько запрос застопорит event loop. Она
устаревает тогда и только тогда, когда меняется сам асинхронный движок.

Через это пришлось пройти дважды. Первая версия эксперимента замедляла
общую внешнюю зависимость — дорожали оба движка одинаково, относительный
порядок не менялся, приспосабливаться было не к чему. Вторая версия
ухудшала синхронный движок — но от этого оценка блокировки event loop не
становится неверной, и поправке снова нечего было исправлять. Обе версии
давали отрицательный результат, который говорил об эксперименте, а не о
методе.

Здесь фоновая нагрузка запускается внутри **асинхронного** движка. Она
отбирает процессорное время у event loop, поэтому каждое вычисление
держит его дольше, чем показало профилирование, и снятая заранее оценка
блокировки оказывается заниженной — ровно та ситуация, ради которой
поправка и вводилась.
...
ASYNC_NOISE = "http://127.0.0.1:8202/noise"

SOURCE = os.environ.get("DRIFT_SOURCE", "neural")
BUDGET = os.environ.get("DRIFT_BUDGET", "60")
# Четыре способа отнестись к изменению условий плюс балансировщик,
# которому нечему устаревать, потому что он ни на что не опирается.
POLICIES = {
    f"budget_{SOURCE}_{BUDGET}": "оценки неизменны",
    f"adabudget_{SOURCE}_{BUDGET}": "поправка по разности с очередью",
    f"health_{SOURCE}_{BUDGET}": "состояние движка, сравнение по блокировке",
    f"healthfull_{SOURCE}_{BUDGET}": "состояние движка, сравнение по полной цене",
...
def set_noise(threads: int) -> None:
    httpx.post(ASYNC_NOISE, params={"threads": threads}, timeout=10)
...
async def main() -> None:
    ...
    print(f"Конкурентность {CONCURRENCY}. Фаза 1 — обычный режим, "
          f"фаза 2 — в асинхронном движке {NOISE_THREADS} фоновых потока.\n")
